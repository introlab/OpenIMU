from qt.resources.ui.python.StreamWindow_ui import Ui_StreamWindow
...
class StreamWindow(QDialog):
    ...
    def browse_requested(self):
        file = QFileDialog().getExistingDirectory(
            caption=self.tr("Select raw data target directory")
        )
    ...
    def edit_requested(self):
        ...
        msg.setText(
            self.tr(
                "While editing settings, no data will be able to be transferred, and all in progress "
    ...
    def save_settings(self):
        ...
        settings.streamer_delete_data_after_transfer = (
            self.UI.chkDeleteFiles.isChecked()
        )
    ...
    def init_streamer(self):
        ...
        for file in FileManager.get_file_list(from_path=save_path):
            ...
            self.add_file_completed(
                device_name=device_name, filename=filename, filesize=file_size
            )

        if self.UI.tabStreamers.currentIndex() == 0:  # Apple Watch
            self.streamer = AppleWatchStreamer(
                port=self.UI.spinPort.value(), path=save_path, parent=self
            )
    ...
    def exec(self):
        ...
        if self.streamer:
            ...
            self.add_to_log(
                self.tr("Unknown streamer type! Aborting."), LogTypes.LOGTYPE_ERROR
            )
    ...
    def update_current_transfer_tab(self):
        self.UI.tabInfos.setTabText(
            0, self.tr("In progress") + " (" + str(self.UI.tableFiles.rowCount()) + ")"
        )
    ...
    def add_file_completed(self, device_name: str, filename: str, filesize: int):
        ...
        self.UI.tabInfos.setTabText(
            1, self.tr("Completed") + " (" + str(self.UI.tableReceived.rowCount()) + ")"
        )
    ...
    def add_file_error(self, filename: str, errorstr: str):
        ...
        self.UI.tabInfos.setTabText(
            2, self.tr("Errors") + " (" + str(self.UI.tableErrors.rowCount()) + ")"
        )
        ...
        if index >= 0:
            ...
            size_cell.setText(
                FileManager.format_file_size(
                    file_size=value, no_suffix=True, ref_size=max_value
                )
                + " / "
    ...
    def streaming_file_error(self, device_name: str, filename: str, error_str: str):
        ...
        # Update device list
        device_item = self.get_device_item(
            device_name=device_name, create_if_absent=False
        )
        if device_item:
    ...
    def get_device_item(
        self, device_name: str, create_if_absent: bool
    ) -> QListWidgetItem:
        device_item = None
    ...
    def device_connected(self, device_name: str, state: bool):
        if state:
            self.add_to_log(
                device_name + ": " + self.tr("connected"), LogTypes.LOGTYPE_INFO
            )
            ...
            self.add_to_log(
                device_name + ": " + self.tr("disconnected"), LogTypes.LOGTYPE_INFO
            )
    ...
    def streaming_file_completed(
        self, device_name: str, file_name: str, file_size: int
    ):
        # Remove file from current list
        ...
        self.add_file_completed(
            ...
            filename=file_name.replace("/", os.sep),
            filesize=file_size,
        )
