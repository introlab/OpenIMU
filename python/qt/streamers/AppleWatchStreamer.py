from qt.streamers.BaseStreamer import BaseStreamer
...
class AppleWatchStreamer(BaseStreamer):
    ...
    def run(self):
        self.add_log.emit(
            self.tr("Starting Sensorlogger (Apple Watch) server on port")
            + " "
        ...
        self.server = ThreadingHTTPServer(
            (self.get_local_ip_address(), self.server_port), self.request_handler
        )
        self.server_running = True
        self.server.timeout = (
            5  # 5 seconds timeout should be ok since we are usually on local network
        )
    ...
    def stop_server(self):
        self.add_log.emit(
            self.tr("Stopping Sensorlogger (Apple Watch) server..."),
            LogTypes.LOGTYPE_INFO,
        )
    ...
    def do_POST(self):
        ...
        if content_type != "cdrv-cmd/File-Upload":
            self.streamer.add_log.emit(
                self.tr("Command refused") + ": " + content_type,
        ...
        if None in [file_type, device_type, device_name, file_path, file_name]:
            self.streamer.add_log.emit(
                self.tr("Badly formatted request. Refused."), LogTypes.LOGTYPE_ERROR
            )
        ...
        destination_dir = (
            (self.streamer.server_save_path + "/" + device_name + "/" + file_path + "/")
            .replace("//", "/")
            .replace("/", os.sep)
        )
        ...
        if file_infos.exists():
            ...
            if file_infos.st_size < content_length:
                self.streamer.add_log.emit(
                    file_name
                    + " - "
                    + self.tr("Existing file, but incomplete")
                    + " ("
                    ...
                    + self.tr("bytes")
                    + " - "
                    + self.tr("retransferring"),
                    LogTypes.LOGTYPE_WARNING,
                )
            else:
                self.streamer.add_log.emit(
                    file_name + " - " + self.tr("Existing file - overwriting"),
                    LogTypes.LOGTYPE_WARNING,
                )
        ...
        if file_type.lower() in ["data", "dat", "csv", "txt", "oimi"]:
            ...
            while content_size_remaining > 0 and self.streamer.server_running:
                ...
                try:
                    ...
                    if not err_desc and len(err.args) > 0:
                        err_desc = err.args[0]
                    self.streamer.file_error_occured.emit(
                        device_name, file_name, str(err_desc)
                    )
                    return
                ...
                if pc != last_pc:
                    self.streamer.update_progress.emit(
                        file_name,
                        " (" + str(content_received) + "/ " + str(content_length) + ")",
            ...
            self.streamer.file_error_occured.emit(
                device_name,
                file_name,
                self.tr("Unsupported file type") + ": " + file_type.lower(),
            )
            self.send_response(400)
        ...
        if file_infos.st_size < content_length:
            # Missing data?!?!
            error = (
                self.tr("Error receiving")
                + ": "
                + str(file_infos.st_size)
                + " "
                + self.tr("bytes received, expected")
                + " "
                + str(content_length)
            )
            ...
            # All is good!
            self.streamer.add_log.emit(
                device_name + ": " + file_name + " - " + self.tr("Completed"),
                LogTypes.LOGTYPE_DONE,
            )
            self.streamer.transfer_completed.emit(
                device_name, file_name, file_infos.st_size
            )
        self.send_response(200)
        self.send_header("Content-type", "file-transfer/ack")
        self.end_headers()
