...
from qt.resources.ui.python.ProgressDialog_ui import Ui_ProgressDialog
...
class WorkerTask(QObject):
    ...
    def process(self):
        self.log_request.emit(
            "Empty task - " + self.title + " - nothing to process!",
        ...
        for task in self.tasks:
            ...
            try:
                ...
                self.task_error.emit(
                    context, str(traceback.format_exception(e, limit=-1))
                )
...
class ProgressDialog(QDialog):
    ...
    def __init__(self, bg_process: BackgroundProcess, job_title: string, parent=None):
        ...
        if parent:
            screen_geometry = (
                parent.window().windowHandle().screen().availableGeometry()
            )
        else:
        ...
        bg_process.update_current_task_progress.connect(
    ...
    def timerEvent(self, a0: "QTimerEvent"):
        ...
        if self.total_work_done > 0:
            ...
            if len(self.work_speed_estimates) > min(10, int(self.total_work_load / 25)):
                mean_speed = sum(self.work_speed_estimates) / len(
                    self.work_speed_estimates
                )
                remaining_time = (
                    int(mean_speed) * (self.total_work_load - self.total_work_done)
                    - elapsed_time
                )
                self.work_speed_estimates.pop(0)
                self.UI.lblRemainingValue.setText(
                    self.format_time_display(remaining_time)
                )
