...
from qt.resources.ui.python.GraphWidget_ui import Ui_frmGraphWidget
from qt.Charts import IMUChartView
from qt.BeaconsView import BeaconsView
from qt.PromptsView import PromptsView
from qt.BaseGraph import GraphInteractionMode
from qt.DataInfosWidget import DataInfosWidget
...
class GraphWindow(QWidget):
    ...
    def __init__(self, graph_type: GraphType, sensor: Sensor, parent=None):
        ...
        if graph_type == GraphType.MAP:
            if platform.system() == "Darwin":
                ...
                return
            from qt.GPSView import GPSView
    ...
    def zoom_area_request(self):
        if self.graph:
            ...
            self.zoomAreaRequested.emit(
                self.graph.get_displayed_start_time(),
                self.graph.get_displayed_end_time(),
            )
        self.update_zoom_buttons_state()

    def zoom_area_request_time(
        self, start_time: float, end_time: float, emit_signal=False
    ):
        if self.graph:
            self.graph.set_selection_area_from_time(start_time, end_time, emit_signal)
            self.graph.zoom_area()
