...
from qt.resources.ui.python.ResultWidget_ui import Ui_frmResult
...
class ResultWindow(QWidget):
    ...
    def update_data(self):
        ...
        self.UI.lblTimeValue.setText(
            str(self.data.processed_time.strftime("%d-%m-%Y %H:%M:%S"))
        )
    ...
    def copy_data_to_clipboard(self, selected_only=False):
        self.copy_to_clipboard(
            source_table=self.UI.tableData, selected_only=selected_only
        )
    ...
    def eventFilter(self, target: QObject, event: QEvent) -> bool:
        if target == self.UI.tableData or target == self.UI.tableParams:
            if isinstance(event, QKeyEvent):
                if event.type() == QEvent.Type.KeyRelease and event.matches(
                    QKeySequence.Copy
                ):
