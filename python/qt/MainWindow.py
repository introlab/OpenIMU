...
from qt.RecordsetWindow import RecordsetWindow
from qt.ResultWindow import ResultWindow
from qt.ImportBrowser import ImportBrowser
from qt.ExportDialog import ExportWindow
from qt.StreamWindow import StreamWindow
from qt.ImportMatchDialog import ImportMatchDialog
from qt.BackgroundProcess import BackgroundProcess, SimpleTask, ProgressDialog
from qt.ProcessSelectWindow import ProcessSelectWindow
...
class MainWindow(QMainWindow):
    ...
    def infos_requested(self):
        ...
        infos_window = ImportWindow(
            dataset=self.currentDataSet, filename=self.currentFileName
        )
    ...
    def process_data_requested(self):
        selector_diag = DataSelectorDialog(
            db_manager=self.dbMan, parent=self, allow_only_one_participant=True
        )
        selector_diag.exec()
    ...
    def db_compact_requested(self):
        ...
        msg.setText(
            self.tr(
                "Database file will be cleaned up and optimized. This can take some time."
            )
            + "\n\r"
    ...
    def new_group_requested(self):
        ...
        if self.dataDialog.exec() == QDialog.Accepted:
            ...
            self.add_to_log(
                self.tr("Group") + " " + group.name + " " + self.tr("added."),
    ...
    def new_participant_requested(self):
        ...
        self.dataWidget = ParticipantWindow(
            db_manager=self.dbMan,
            participant=None,
        ...
        if self.dataDialog.exec() == QDialog.Accepted:
            ...
            self.UI.treeDataSet.select_item(
                item_type="participant", item_id=part.id_participant
            )
            self.add_to_log(
                self.tr("Participant") + " " + part.name + " " + self.tr("added."),
                LogTypes.LOGTYPE_DONE,
            )
    ...
    def tree_item_current_changed(
        self, item: QTreeWidgetItem, previous: QTreeWidgetItem
    ):
        if not item:
            return
        ...
        self.UI.btnRename.setEnabled(
            item_type == "group"
            or item_type == "participant"
            or item_type == "recordset"
            or item_type == "result"
        )
        ...
        if (
            item_type == "recordsets"
            or item_type == "recordset"
            or item_type == "subrecord"
            or item_type == "date"
        ):
            ...
            records_widget = RecordsetWindow(
                manager=self.dbMan, recordset=self.currentRecordsets, parent=self
            )
    ...
    def data_was_saved(self):
        ...
        if item_type == "group":
            ...
            self.update_group(group)
            self.add_to_log(
                self.tr("Group") + " " + group.name + " " + self.tr("updated."),
                LogTypes.LOGTYPE_DONE,
            )
        ...
        if item_type == "participant":
            ...
            self.update_participant(part)
            self.add_to_log(
                self.tr("Participant") + " " + part.name + " " + self.tr("updated."),
                LogTypes.LOGTYPE_DONE,
            )
    ...
    def delete_requested(self):
        ...
        if rval == QMessageBox.Yes:
            ...
            if item_type == "group":
                ...
                task = SimpleTask(
                    self.tr("Deleting") + ' "' + group.name + '" ',
                    self.dbMan.delete_group,
                    group,
                )
                tasks.append(task)
            ...
            if item_type == "participant":
                ...
                task = SimpleTask(
                    self.tr("Deleting") + ' "' + part.name + '" ',
                    self.dbMan.delete_participant,
                    part,
                )
                tasks.append(task)
            ...
            if item_type == "recordset":
                    ...
                    if result is not None:
                        for ref in result.processed_data_ref:
                            if ref.recordset.id_recordset == item_id:
                                ...
                                task = SimpleTask(
                                    self.tr("Deleting") + ' "' + result.name + '" ',
                                    self.dbMan.delete_processed_data,
                                    result,
                                )
                                tasks.append(task)
                ...
                task = SimpleTask(
                    self.tr("Deleting") + ' "' + recordset.name + '" ',
                    self.dbMan.delete_recordset,
                    recordset,
                )
                tasks.append(task)
            ...
            if item_type == "date":
                ...
                search_date = self.UI.treeDataSet.dates[
                    TreeDataWidget.get_date_id(
                        self.UI.treeDataSet.currentItem().text(0), id_participant
                    )
                ]
            ...
            self.add_to_log(
                item_name + " " + self.tr("was deleted."), LogTypes.LOGTYPE_DONE
            )
            self.clear_main_widgets()
    ...
    def transfer_requested(self):
        ...
        if stream_diag.folders_to_import:
            # Match windows
            matcher = ImportMatchDialog(
                dbmanager=self.dbMan, datas=stream_diag.folders_to_import, parent=self
            )
            if matcher.exec() == QDialog.Accepted:
                ...
                for file_name, file_part in files.items():
                    ...
                    file_info = {
                        "participant": file_part,
                        "file_name": file_name,
                        "file_type": importer_id,
                    }
                ...
                if not stream_diag.get_delete_files_after_import():
                    ...
                    target_dir = (
                        stream_diag.get_base_data_save_path() + os.sep + "Imported"
                    )
                    FileManager.merge_folders(
                        stream_diag.get_data_save_path(), target_dir
                    )
