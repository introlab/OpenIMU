from qt.resources.ui.python.StartDialog_ui import Ui_StartDialog

from qt.ImportWindow import ImportWindow
from qt.ImportDialogWizard import ImportDialogWizard
...
class StartWindow(QDialog):
    ...
    def open_clicked(self):
        file_diag = QFileDialog.getOpenFileName(
            caption=self.tr("Filename to open"),
            filter="*.oi",
    ...
    def open_file(self, filename: str) -> bool:
        ...
        if not path.exists(filename):
            QMessageBox.warning(
                self,
                self.tr("File missing"),
                filename
                + " "
                + self.tr(
                    "doesn't exist. Perhaps it has been moved, deleted or " "renamed?"
                ),
            )
            return False
