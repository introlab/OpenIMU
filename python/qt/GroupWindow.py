...
from qt.resources.ui.python.GroupWidget_ui import Ui_frmGroup
...
class GroupWindow(DataEditor):
    ...
    def update_modified_status(self):
        has_changes = (
            (self.group is not None and self.UI.txtName.text() != self.group.name)
            or (self.group is None and self.UI.txtName.text() != "")
            or (
                self.group is not None
                and self.UI.txtDesc.toPlainText() != self.group.description
            )
            or (self.group is None and self.UI.txtDesc.toPlainText() != "")
        )
