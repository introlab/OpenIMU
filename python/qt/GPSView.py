...
class GPSView(QWebEngineView, BaseGraph):
    ...
    def add_position(self, timestamp, latitude, longitude):
        ...
        if self.pageReady is True:
            self.page().runJavaScript(
                "addPosition(" + str(latitude) + "," + str(longitude) + ");"
            )
        else:
    ...
    def set_selection_area_from_time(self, start_time, end_time, emit_signal=False):
        ...
        try:
            ...
            # Find nearest point
            start_pos = (
                np.abs(np.asarray(list(self.positions.keys())) - start_time)
            ).argmin()
        ...
        try:
            ...
            # Find nearest point
            end_pos = (
                np.abs(np.asarray(list(self.positions.keys())) - end_time)
            ).argmin()
        ...
        if start_pos is not None and end_pos is not None:
            if self.pageReady:
                for i in range(start_pos, end_pos):
                    self.page().runJavaScript(
                        "addSelectedPosition("
                        + str(list(self.positions.values())[i].x())
                        + ","
    ...
    def page_loaded(self, state):
        ...
        if state is True:
            ...
            for coords in self.path:
                self.page().runJavaScript(
                    "addPosition(" + str(coords[0]) + "," + str(coords[1]) + ");"
                )
            if self.marker_position:
                self.page().runJavaScript(
                    "setMarkerPosition("
                    + str(self.marker_position[0])
                    + ","
                    + str(self.marker_position[1])
                    + ");"
                )
