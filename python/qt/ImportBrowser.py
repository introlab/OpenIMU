...
class ImportBrowser(QDialog):
    ...
    def __init__(self, data_manager, parent=None):
        ...
        self.UI.cmbParticipant.currentIndexChanged.connect(
            self.on_participant_combobox_changed
        )
    ...
    def do_import(self, file_list: list[dict]):
        class Importer(WorkerTask):
            ...
            def load_data(self):
                ...
                if not DataSource.datasource_exists_for_participant(
                    filename=self.short_filename,
                    participant=self.importer.participant,
                    md5=file_md5,
            ...
            def process(self):
                ...
                if not DataSource.datasource_exists_for_participant(
                    ...
                    self.log_request.emit(
                        self.tr("Loading file:") + ' "' + self.short_filename + '"',
                    ...
                    if self.results is not None:
                        self.log_request.emit(
                            self.tr("Importing data..."), LogTypes.LOGTYPE_INFO
                        )
                        ...
                        for recordset in self.importer.recordsets:
                            if not DataSource.datasource_exists_for_recordset(
                                filename=self.short_filename,
                                recordset=recordset,
                                md5=file_md5,
                                db_session=self.importer.db.session,
                        ...
                        self.log_request.emit(
                            self.tr("File import completed."), LogTypes.LOGTYPE_DONE
                        )
                    else:
                        self.log_request.emit(
                            self.tr("Error loading file:")
                            + " "
                            + self.importer.last_error,
                            LogTypes.LOGTYPE_ERROR,
                        )
                else:
                    self.log_request.emit(
                        self.tr("Data from file")
                        + ' "'
                        + self.short_filename
                        + '" '
                        + self.tr("already in the database for participant")
                        + ' "'
                        + self.importer.participant.name
                        + '" - '
                        + self.tr("ignored."),
        ...
        for to_import in file_list:
            if (
                "participant" not in to_import
                or "file_type" not in to_import
                or "file_name" not in to_import
            ):
                self.log_request.emit(
                    self.tr("Invalid import structure") + ": " + str(to_import),
                    LogTypes.LOGTYPE_WARNING,
                )
                continue
        ...
        if self.has_error:
            ...
            QMessageBox.critical(
                self,
                self.tr("Error while importing data"),
                self.tr(
                    "Error occured while importing data files. See logs for more information."
                ),
            )
            self.has_error = False
    ...
    def process_error_occurred(self, filename: str, error_msg: str):
        self.log_request.emit(
            self.tr("Error importing file:") + ' "' + filename + '" - ' + error_msg,
            LogTypes.LOGTYPE_ERROR,
        )
        self.has_error = True

    def add_file_to_list(
        self, filename: str, filetype_id: int = -1, file_participant=None
    ):
        table = self.UI.tableFiles
        ...
        if file_participant:
            ...
            item_combo.setCurrentIndex(
                item_combo.findText(self.UI.cmbParticipant.currentText())
            )
        self.validate_row_combobox(item_combo)
    ...
    def add_dir_to_list(self, dir_path: str):
        ...
        dir_browser = QDirIterator(
            dir_path, [], QDir.Files | QDir.NoDotAndDotDot, QDirIterator.Subdirectories
        )
        files = []
    ...
    @staticmethod
    def add_participant_in_combobox(
        participant: Participant, combobox: QComboBox, include_group=False
    ):
    ...
    def add_file_clicked(self):
        ...
        files = QFileDialog.getOpenFileNames(
            parent=self,
            caption=self.tr("Select file(s) to import"),
    ...
    def add_dir_clicked(self):
        ...
        dirs = QFileDialog.getExistingDirectory(
            parent=self,
            caption=self.tr("Select folder to import"),
            dir=settings.data_load_path,
        )
        if not dirs:
            return
    ...
    def on_add_participant_clicked(self):
        ...
        if self.part_diag.exec() == QDialog.Accepted:
            ...
            self.add_participant_in_combobox(
                part, self.UI.cmbParticipant, include_group=True
            )
            ...
            for i in range(self.UI.tableFiles.rowCount()):
                ...
                self.add_participant_in_combobox(
                    participant=part, combobox=combobox, include_group=True
                )

    @Slot()
    def update_buttons_states(self):
        self.UI.btnOK.setEnabled(
            len(self.invalid_controls) == 0 and self.UI.tableFiles.rowCount() > 0
        )
        self.UI.btnDelFile.setEnabled(
            self.UI.tableFiles.rowCount() > 0
            and len(self.UI.tableFiles.selectedItems()) > 0
        )
