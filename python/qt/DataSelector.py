...
from qt.resources.ui.python.DataSelector_ui import Ui_DataSelector
...
class DataSelector(QWidget):
    ...
    def __init__(
        ...
        allow_only_one_participant=False,
        show_results=False,
        parent=None,
    ):
    ...
    def add_recordset(self, recordset: Recordset) -> QTreeWidgetItem:
        ...
        if date_item is None:
            date_item = self.add_date(
                recordset.start_timestamp, recordset.id_participant
            )
    ...
    def add_date(self, date_update: datetime, id_parent_part: int) -> QTreeWidgetItem:
        ...
        date_text_id = TreeDataWidget.get_date_id(
            date_text=date_text, id_parent_part=id_parent_part
        )
        ...
        if part_item is not None:
            ...
            print(
                "No participant found in treeview for participant id="
                + str(id_parent_part)
            )
    ...
    def get_all_selected(self) -> dict:  # Return ids of selected items
        rval = {
            "groups": [group.id_group for group in self.get_selected_groups()],
            "participants": [
                part.id_participant for part in self.get_selected_participants()
            ],
