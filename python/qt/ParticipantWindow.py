...
class ParticipantWindow(DataEditor):
    ...
    def __init__(
        ...
        if default_group is not None:
            self.UI.cmbGroups.setCurrentIndex(
                self.UI.cmbGroups.findData(default_group.id_group, Qt.UserRole)
            )
    ...
    def update_modified_status(self):
        has_changes = (
            ...
            or (self.participant is None and self.UI.txtName.text() != "")
            or (
                self.participant is not None
                and self.UI.txtDesc.toPlainText() != self.participant.description
            )
            or (self.participant is None and self.UI.txtDesc.toPlainText() != "")
            or (
                self.participant is not None
