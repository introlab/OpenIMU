...
class RecordsetWindow(QWidget):
    ...
    def __init__(self, manager, recordset: list, parent=None):
        ...
        # Variables
        self.time_pixmap = (
            False  # Flag used to check if we need to repaint the timeline or not
        )
        ...
        self.UI.graphSensorsTimeline.fitInView(
            self.timeSensorsScene.sceneRect(), Qt.KeepAspectRatio
        )
        ...
        self.UI.btnTimeZoomSelection.clicked.connect(
            self.on_timeview_zoom_selection_requested
        )
    ...
    def refresh_timeview(self):
        ...
        if len(self.recordsets) > 0:
            num_days = (
                self.get_recordset_end_day_date() - self.get_recordset_start_day_date()
            ).days
            ...
            if num_days < 365 * 2:
                ...
                message.warning(
                    ...
                    self.tr("Warning"),
                    self.tr(
                        "Displaying a timeline spanning over more than 2 "
    ...
    def adjust_timeview_size(self):
        ...
        if (
            self.timeScene.itemsBoundingRect().width() * self.zoom_level
            > self.UI.graphTimeline.width()
        ):
            ...
            self.UI.scrollTimeline.setMaximum(
                self.timeScene.itemsBoundingRect().width() * self.zoom_level
            )
                ...
                for sensor in self.dbMan.get_sensors(recordset):
                    ...
                    if sensor.id_sensor not in self.sensors:
                        ...
                        sensor_item = QAction(
                            sensor.name
                            + " ["
                            + "{:.1f}".format(sensor.sampling_rate)
                            + " Hz]"
                        )
    ...
    def get_time_from_timeview_pos(self, pos):
        ...
        current_time = (pos / self.timeScene.width()) * (
            end_time - start_time
        ) + start_time
    ...
    def draw_dates(self):
        ...
        self.timeSensorsScene.addRect(
            0, 0, self.timeSensorsScene.width(), 20, black_pen, blue_brush
        )
    ...
    def draw_grid(self):
        ...
        for location in self.sensors_location:
            ...
            for _ in sensors:
                self.timeScene.addLine(
                    0, pos, self.timeScene.width() - 1, pos, hgrid_pen
                )
                self.timeSensorsScene.addLine(
                    0, pos, self.timeSensorsScene.width(), pos, hgrid_pen
                )
                pos += 20
        ...
        self.timeSensorsScene.addLine(
            0, pos, self.timeSensorsScene.width() - 1, pos, hgrid_pen
        )
    ...
    def draw_recordsets(self):
        ...
        for record in self.recordsets:
            ...
            self.timeScene.addRect(
                ...
                21,
                span,
                self.timeSensorsScene.height() - 21,
                recordset_pen,
                recordset_brush,
            )
    ...
    def draw_sensors_names(self):
        ...
        for location in self.sensors_location:
            ...
            for sensor_id in sensors:
                ...
                # Sensor names
                label = self.timeSensorsScene.addText(
                    sensor.name + " [" + "{:.1f}".format(sensor.sampling_rate) + " Hz]"
                )
                label.setPos(0, pos)
        ...
        self.UI.graphSensorsTimeline.setMaximumWidth(
            self.timeSensorsScene.itemsBoundingRect().width()
        )
        ...
        for location in self.sensors_location:
            ...
            for sensor_id in sensors:
                for record in self.recordsets:
                    tasks.append(
                        DBSensorTimesTask(
                            title=self.tr("Loading temporal data") + "...",
                            db_manager=self.dbMan,
                            sensor_id=sensor_id,
                            recordset=record,
                        )
                    )
    ...
    def draw_timebar(self):
        ...
        line_pen.setWidth(2)
        self.time_bar = self.timeScene.addLine(
            1, 21, 1, self.timeScene.height() - 1, line_pen
        )
    ...
    def get_sensor_graph_type(sensor):
        if (
            sensor.id_sensor_type == SensorType.ACCELEROMETER
    ...
    def sensor_graph_selected(self, sensor_item):
        ...
        if sensor_item.isChecked():
            ...
            timeseries, channel_data = self.get_sensor_data(
                sensor
                ...
                if platform.system() != "Darwin":
                    for data in channel_data:
                        ...
                        if gps.latitude != 0 and gps.longitude != 0:
                            graph_window.graph.add_position(
                                data.timestamps.start_timestamp,
                                gps.latitude / 1e7,
                                gps.longitude / 1e7,
                            )
                            graph_window.set_cursor_position_from_time(
                                data.timestamps.start_timestamp
                            )
                else:
            ...
            if graph_type == GraphType.BEACON:
                for series in timeseries:
                    if (
                        series.__contains__("x")
                        and series.__contains__("y")
                        and series.__contains__("label")
                    ):
            ...
            if graph_type == GraphType.QUESTIONS:
                ...
                for series in timeseries:
                    ...
                    for answer in series["y"]:
                        graph_window.graph.add_answer(
                            np.array(
                                [
                                    series["x"][current_index * 2],
                                    series["x"][current_index * 2 + 1],
                                ]
                            ),
                            answer,
                        )
            ...
            if graph_window is not None:
                ...
                subwindow.setObjectName(
                    "GraphWindow"
                )  # Name required to set the proper background style!
                ...
                if graph_window.graph is not None:
                    ...
                    graph_window.graph.selectedAreaChanged.connect(
                        self.graph_selected_area_changed
                    )
                    graph_window.graph.clearedSelectionArea.connect(
                        self.on_timeview_clear_selection_requested
                    )
    ...
    def graph_cursor_changed(self, timestamp):
        ...
        self.UI.lblCursorTime.setText(
            datetime.fromtimestamp(current_time).strftime("%d-%m-%Y %H:%M:%S")
        )

    def ensure_time_bar_visible(self, pos):
        if self.UI.scrollTimeline.isVisible():
            max_visible_x = (
                self.UI.graphTimeline.mapToScene(self.UI.graphTimeline.rect())
                .boundingRect()
                .x()
                + self.UI.graphTimeline.mapToScene(self.UI.graphTimeline.rect())
                .boundingRect()
    ...
    def timeview_selected(self, start_x, end_x):
        ...
        self.selection_rec = self.timeScene.addRect(
            start_x,
            20,
            end_x - start_x,
            self.timeScene.height() - 20,
            selection_pen,
            selection_brush,
        )
        ...
        for graph in self.sensors_graphs.values():
            if graph is not None:
                graph.set_selection_area_from_time(
                    self.get_time_from_timeview_pos(start_x),
    ...
    def on_timeview_zoom_selection_requested(self):
        ...
        self.UI.scrollTimeline.setValue(
            (self.selection_rec.rect().x() + self.selection_rec.rect().width() / 2)
            * self.zoom_level
        )
        self.on_timeview_clear_selection_requested()
