...
class ExportWindow(QDialog):
    ...
    def __init__(self, db_manager: DBManager, parent=None):
        ...
        self.data_selector = DataSelector(
            db_manager=self.dbMan, parent=self, show_results=True
        )
    ...
    def directory_selection_clicked(self):
        directory = QFileDialog().getExistingDirectory(
            caption=self.tr("Select base folder for exported files"),
    ...
    def export(self):
        ...
        class FileExporter(WorkerTask):
            def __init__(self, _dbexporter: DBExporter, _datatoexport: dict):
                ...
                self.total_count = (
                    len(self.dataList["groups"])
                    + len(self.dataList["participants"])
                    + len(self.dataList["recordsets"])
                    + len(self.dataList["results"])
                )

                self.dbExporter = _dbexporter
                super().__init__(
                    self.tr("Exporting to format")
                    + ": "
                    + ExporterTypes.value_names[self.dbExporter.exportFormat],
                    self.total_count,
                )
            ...
            def process(self):
                ...
                for id_recordset in self.dataList["recordsets"]:
                    self.change_task_title.emit(
                        title + " (" + str(current) + " / " + str(total_count) + ")"
                    )
        ...
        dialog = ProgressDialog(
            process, self.tr("Exporting to format") + ": " + file_format, self
        )
        ...
        if self.has_export_errors:
            ...
            QMessageBox.critical(
                self,
                self.tr("Error while exporting data"),
                self.tr(
                    "Error occurred while exporting data files. See logs for more information."
                ),
            )
            self.has_export_errors = False
    ...
    def process_error_occurred(self, context: str, error_msg: str):
        self.log_request.emit(
            self.tr("Error exporting data:") + ' "' + context + '" - ' + error_msg,
