...
from qt.BaseGraph import BaseGraph, GraphInteractionMode
...
class IMUChartView(QChartView, BaseGraph):
        ...
        if series_id < len(self.chart.series()):
            ...
            try:
                end_index = current_points.index(
                    QPointF(xdata[len(xdata) - 1] * 1000, ydata[len(ydata) - 1])
                )  # Right on!
                # print("update_data: start_index found exact match.")
            except ValueError:
                # print("update_data: start_index no exact match - scanning deeper...")
    ...
    def add_test_data(self):
        ...
        xdata = np.linspace(0.0, 10.0, npoints)
    ...
    def set_cursor_position(self, pos, emit_signal=False):
        ...
        for i in range(self.ncurves):
            ...
            if i == 0 and xmap_initial >= 0:
                display += (
                    "<i>"
                    + (datetime.datetime.fromtimestamp(xmap_initial / 1000)).strftime(
                        "%d-%m-%Y %H:%M:%S:%f"
                    )
                    + "</i>"
                )
    ...
    def resizeEvent(self, e: QResizeEvent):
        ...
        # Update selection
        if self.selection_rec:
            self.set_selection_area_from_time(
                self.selection_start_time, self.selection_stop_time
            )
