...
class ActigraphImporter(BaseImporter):
    ...
    def get_recordset(self, timestamp, session_name=str()):
        ...
        recordset = self.db.add_recordset(
            self.participant, session_name, my_time, my_time, True
        )
        ...
        for timestamp in all_timestamps:
            ...
            for index in range(0, len(value_dict[timestamp])):
                ...
                timevect = np.linspace(
                    timestamp,
                    timestamp + len(value_dict[timestamp][index]),
                    num=len(vector),
                    endpoint=False,
                    dtype=np.float64,
                )
    ...
    def import_battery_to_database(self, info, battery: dict):
        # Create sensor
        volt_sensor = self.add_sensor_to_db(
            SensorType.BATTERY, "Battery", info["Device Type"], "General", 10, 1
        )

        # Create channel
        volt_channel = self.add_channel_to_db(
            volt_sensor, Units.VOLTS, DataFormat.FLOAT32, "Battery"
        )
        ...
        for epoch in battery:
            ...
            timevect = np.linspace(
                epoch[0], epoch[0] + 1, num=1, endpoint=False, dtype=np.float64
            )
            # Create sensor timestamps first
            sensor_timestamps = SensorTimestamps()
            sensor_timestamps.timestamps = timevect
            ...
            self.add_sensor_data_to_db(
                recordset, volt_sensor, volt_channel, sensor_timestamps, value
            )
    ...
    def import_lux_to_database(self, info, lux: dict):
        # Create sensor
        lux_sensor = self.add_sensor_to_db(
            SensorType.LUX, "Lux", info["Device Type"], "Unknown", 0, 1
        )

        # Create channel
        lux_channel = self.add_channel_to_db(
            lux_sensor, Units.LUX, DataFormat.FLOAT32, "Lux"
        )
        ...
        for epoch in lux:
            ...
            self.add_sensor_data_to_db(
                recordset, lux_sensor, lux_channel, sensor_timestamps, value
            )
    ...
    def import_sensor_data_to_database(self, info, sensor_data: dict):
        ...
        for sensor in sensor_names:
            ...
            if sensor == "Temperature":
                # Create sensor
                temp_sensor = self.add_sensor_to_db(
                ...
                temp_channel = self.add_channel_to_db(
                    temp_sensor, Units.CELCIUS, DataFormat.FLOAT32, "Temperature"
                )
        ...
        split_indexes.extend(
            [index + 1 for index, value in enumerate(np.diff(data_times)) if value != 1]
        )
        ...
        if "Accelerometer" in sensor_names:
            ...
            for record_index in range(1, len(split_indexes)):
                ...
                for index in range(0, len(accelerometer_channels)):
                    values = np.asarray(
                        [row[1][index] for row in data_recordset], dtype=np.float32
                    )
                    self.add_sensor_data_to_db(
                        recordset,
                        accelerometer_sensor,
        ...
        if "Temperature" in sensor_names:
            ...
            for record_index in range(1, len(split_indexes)):
                ...
                values = np.asarray(
                    [row[1] for row in data_recordset], dtype=np.float32
                )
                self.add_sensor_data_to_db(
                    recordset, temp_sensor, temp_channel, sensor_timestamps, values
                )
