...
class AppleWatchImporter(BaseImporter):
                ...
                with open(filename, "rb") as file:
                    ...
                    if os.path.exists(session_file_name):
                        with open(session_file_name, "r") as session_file:
                            ...
                            if session_info.__contains__(
                                "participant"
                            ) and session_info.__contains__("timestamp"):
                                self.session_name = (
                                    session_info["timestamp"]
                                    + "_"
                                    + session_info["participant"]
                                )
    ...
    def import_activity_to_database(self, settings: str, activity: dict):
        activity_sensor = self.add_sensor_to_db(
            SensorType.ACTIVITY,
            "Activity",
            "AppleWatch",
            "Wrist",
        ...
        confidence_channel = self.add_channel_to_db(
            activity_sensor, Units.NONE, DataFormat.UINT8, "Confidence"
        )
        car_channel = self.add_channel_to_db(
            activity_sensor, Units.NONE, DataFormat.UINT8, "Automotive"
        )
        cycle_channel = self.add_channel_to_db(
            activity_sensor, Units.NONE, DataFormat.UINT8, "Cycling"
        )
        run_channel = self.add_channel_to_db(
            activity_sensor, Units.NONE, DataFormat.UINT8, "Running"
        )
        stat_channel = self.add_channel_to_db(
            activity_sensor, Units.NONE, DataFormat.UINT8, "Stationnary"
        )
        walk_channel = self.add_channel_to_db(
            activity_sensor, Units.NONE, DataFormat.UINT8, "Walking"
        )
        unknown_channel = self.add_channel_to_db(
            activity_sensor, Units.NONE, DataFormat.UINT8, "Unknown"
        )
        ...
        for timestamp in activity:
            ...
            recordset = self.get_recordset(
                timestamp.timestamp(), session_name=self.session_name
            )
            ...
            if len(timesarray) == 0:
                self.notify_error("No temporal data")
                return
            ...
            self.add_sensor_data_to_db(
                recordset, activity_sensor, car_channel, sensor_timestamps, masked[:, 0]
            )
            ...
            self.add_sensor_data_to_db(
                recordset, activity_sensor, run_channel, sensor_timestamps, masked[:, 0]
            )
            ...
            count += 1
            self.notify_progress(50 + np.floor(count / len(activity) / 2 * 100))

    def import_raw_accelerometer_to_database(
        self, settings: str, sample_rate, raw_accelero: dict
    ):
        # DL Oct. 17 2018, New import to database
        raw_accelerometer_sensor = self.add_sensor_to_db(
        ...
        # Create channels
        raw_accelerometer_channels.append(
        ...
        for timestamp in raw_accelero:
            ...
            valuesarray = np.asarray(
                raw_accelero[timestamp]["values"], dtype=np.float32
            )
            ...
            for i, channel in enumerate(raw_accelerometer_channels):
                ...
                self.notify_progress(
                    ...
                    + np.floor(
                        count
                        / (len(raw_accelero) * len(raw_accelerometer_channels))
                        / 2
                        * 100
                    )
                )
    ...
    def import_raw_gyro_to_database(self, settings, sample_rate, raw_gyro: dict):
        # DL Oct. 17 2018, New import to database
        # Create sensor
        raw_gyro_sensor = self.add_sensor_to_db(
        ...
        raw_gyro_channels.append(
            self.add_channel_to_db(
                raw_gyro_sensor, Units.DEG_PER_SEC, DataFormat.FLOAT32, "Gyro_X"
            )
        )

        raw_gyro_channels.append(
            self.add_channel_to_db(
                raw_gyro_sensor, Units.DEG_PER_SEC, DataFormat.FLOAT32, "Gyro_Y"
            )
        )
        ...
        for timestamp in raw_gyro:
            ...
            count += 1
            self.notify_progress(50 + np.floor(count / len(raw_gyro) / 2 * 100))

    def import_raw_magneto_to_database(self, settings, sample_rate, raw_data: dict):
        # Create sensor
        raw_magneto_sensor = self.add_sensor_to_db(
        ...
        # Create channels
        raw_mag_channels.append(
            self.add_channel_to_db(
                raw_magneto_sensor, Units.UTESLA, DataFormat.FLOAT32, "Mag_X"
            )
        )
        raw_mag_channels.append(
            self.add_channel_to_db(
                raw_magneto_sensor, Units.UTESLA, DataFormat.FLOAT32, "Mag_Y"
            )
        )
        raw_mag_channels.append(
            self.add_channel_to_db(
                raw_magneto_sensor, Units.UTESLA, DataFormat.FLOAT32, "Mag_Z"
            )
        )
        ...
        for timestamp in raw_data:
            ...
            count += 1
            self.notify_progress(50 + np.floor(count / len(raw_data) / 2 * 100))

    def import_heartrate_to_database(self, settings, sample_rate, heartrate: dict):
        heartrate_sensor = self.add_sensor_to_db(
        ...
        heartrate_channel = self.add_channel_to_db(
            heartrate_sensor, Units.BPM, DataFormat.UINT8, "Heartrate"
        )
        count = 0
        for timestamp in heartrate:
            ...
            if len(timesarray) == 0:
                self.notify_error("Aucune données temporelles.")
                return
            ...
            count += 1
            self.notify_progress(50 + np.floor(count / len(heartrate) / 2 * 100))

    def import_coordinates_to_database(self, settings, sample_rate, coordinates: dict):
        # DL Oct. 17 2018, New import to database
        coordinates_sensor = self.add_sensor_to_db(
            SensorType.GPS,
            "Coordinates",
            "AppleWatch",
            "Wrist",
        ...
        coordinates_channel = self.add_channel_to_db(
            coordinates_sensor, Units.NONE, DataFormat.UINT8, "Coordinates"
        )
        ...
        for timestamp in coordinates:
            ...
            for i, value in enumerate(valuesarray):
                ...
                self.notify_progress(50 + np.floor(i / len(valuesarray) / 2 * 100))

    def import_pedometer_to_database(self, settings, pedometer: dict):
        pedometer_sensor = self.add_sensor_to_db(
            SensorType.STEP,
            "Pedometer",
            "AppleWatch",
            "Wrist",
        ...
        step_channel = self.add_channel_to_db(
            pedometer_sensor, Units.NONE, DataFormat.UINT32, "Step count"
        )
        distance_channel = self.add_channel_to_db(
            pedometer_sensor, Units.METERS, DataFormat.FLOAT32, "Distance"
        )
        average_pace_channel = self.add_channel_to_db(
            pedometer_sensor, Units.METERS_PER_SEC, DataFormat.FLOAT32, "Average Pace"
        )
        pace_channel = self.add_channel_to_db(
            pedometer_sensor, Units.METERS_PER_SEC, DataFormat.FLOAT32, "Pace"
        )
        cadence_channel = self.add_channel_to_db(
            pedometer_sensor, Units.METERS_PER_SEC, DataFormat.FLOAT32, "Cadence"
        )
        floors_up_channel = self.add_channel_to_db(
            pedometer_sensor, Units.NONE, DataFormat.UINT32, "Floors ascended"
        )
        floors_down_channel = self.add_channel_to_db(
            pedometer_sensor, Units.NONE, DataFormat.UINT32, "Floors descended"
        )
        ...
        for timestamp in pedometer:
            ...
            # if values.min() >= 0 and values.max() >= 0:  # Ignore if not available (all -1)
            self.add_sensor_data_to_db(
                recordset, pedometer_sensor, distance_channel, sensor_timestamps, values
            )
            ...
            # if values.min() >= 0 and values.max() >= 0:  # Ignore if not available (all -1)
            self.add_sensor_data_to_db(
                recordset, pedometer_sensor, pace_channel, sensor_timestamps, values
            )

            values = valuesarray[:, 4]
            # if values.min() >= 0 and values.max() >= 0:  # Ignore if not available (all -1)
            self.add_sensor_data_to_db(
                recordset, pedometer_sensor, cadence_channel, sensor_timestamps, values
            )

            count += 1
            self.notify_progress(50 + np.floor(count / len(pedometer) / 2 * 100))

    def import_sensoria_to_database(self, settings, sample_rate, sensoria: dict):
        # DL Oct. 17 2018, New import to database
        sensoria_acc_sensor = self.add_sensor_to_db(
            SensorType.ACCELEROMETER,
            "Accelerometer",
            "Sensoria",
            "Foot",
        ...
        sensoria_gyro_sensor = self.add_sensor_to_db(
            SensorType.GYROMETER,
            "Gyroscope",
            "Sensoria",
            "Foot",
        ...
        sensoria_gyro_channels.append(
            self.add_channel_to_db(
                sensoria_gyro_sensor, Units.DEG_PER_SEC, DataFormat.FLOAT32, "Gyro_X"
            )
        )
        sensoria_gyro_channels.append(
            self.add_channel_to_db(
                sensoria_gyro_sensor, Units.DEG_PER_SEC, DataFormat.FLOAT32, "Gyro_Y"
            )
        )
        sensoria_gyro_channels.append(
            self.add_channel_to_db(
        ...
        sensoria_mag_sensor = self.add_sensor_to_db(
            SensorType.MAGNETOMETER,
            "Magnetometer",
            "Sensoria",
            "Foot",
        ...
        sensoria_mag_channels.append(
            self.add_channel_to_db(
                sensoria_mag_sensor, Units.GAUSS, DataFormat.FLOAT32, "Mag_X"
            )
        )
        sensoria_mag_channels.append(
            self.add_channel_to_db(
                sensoria_mag_sensor, Units.GAUSS, DataFormat.FLOAT32, "Mag_Y"
            )
        )
        sensoria_mag_channels.append(
            self.add_channel_to_db(
                sensoria_mag_sensor, Units.GAUSS, DataFormat.FLOAT32, "Mag_Z"
            )
        )

        sensoria_fsr_sensor = self.add_sensor_to_db(
            SensorType.FSR,
            "FSR",
            "Sensoria",
            "Foot",
        ...
        sensoria_fsr_channels.append(
            self.add_channel_to_db(
                sensoria_fsr_sensor, Units.NONE, DataFormat.UINT16, "META-1"
            )
        )
        sensoria_fsr_channels.append(
            self.add_channel_to_db(
                sensoria_fsr_sensor, Units.NONE, DataFormat.UINT16, "META-5"
            )
        )
        sensoria_fsr_channels.append(
            self.add_channel_to_db(
                sensoria_fsr_sensor, Units.NONE, DataFormat.UINT16, "HEEL"
            )
        )
        count = 0
        for timestamp in sensoria:
            ...
            count += 1
            self.notify_progress(50 + np.floor(count / len(sensoria) / 2 * 100))

    def import_beacons_to_database(self, settings, sample_rate, beacons: dict):
        # DL Oct. 17 2018, New import to database
        beacons_sensor = self.add_sensor_to_db(
            SensorType.BEACON,
            "Beacons",
            "Kontact",
            "Environment",
        ...
        for timestamp in beacons:
            ...
            for i, timearray in enumerate(timesarray):
                ...
                channel_values[beacon_id].append(
                    (timearray, valuesarray[i][16], valuesarray[i][17])
                )
            ...
            for key in channel_values:
                timevect = np.asarray(
                    [x[0] for x in channel_values[key]], dtype=np.float64
                )
                ...
                # Create channel
                channel_tx_power = self.add_channel_to_db(
                    beacons_sensor, Units.NONE, DataFormat.SINT8, key + "_TxPower"
                )

                channel_rssi = self.add_channel_to_db(
                    beacons_sensor, Units.NONE, DataFormat.SINT8, key + "_RSSI"
                )

                tx_power_vect = np.asarray(
                    [x[1] for x in channel_values[key]], dtype=np.int8
                )
                rssi_vect = np.asarray(
                    [x[2] for x in channel_values[key]], dtype=np.int8
                )
            ...
            count += 1
            self.notify_progress(50 + np.floor(count / len(beacons) / 2 * 100))

    def import_motion_to_database(
        self, settings, sampling_rate, motion: dict, version: int
    ):
        ...
        # Create sensor
        gyro_sensor = self.add_sensor_to_db(
            SensorType.GYROMETER,
            "Gyroscope",
            "AppleWatch",
            "Wrist",
        ...
        gyro_channels.append(
            self.add_channel_to_db(
                gyro_sensor, Units.DEG_PER_SEC, DataFormat.FLOAT32, "Gyro_X"
            )
        )

        gyro_channels.append(
            self.add_channel_to_db(
                gyro_sensor, Units.DEG_PER_SEC, DataFormat.FLOAT32, "Gyro_Y"
            )
        )

        gyro_channels.append(
            self.add_channel_to_db(
                gyro_sensor, Units.DEG_PER_SEC, DataFormat.FLOAT32, "Gyro_Z"
            )
        )

        orientation_sensor = self.add_sensor_to_db(
        ...
        orientation_channels.append(
            self.add_channel_to_db(
                orientation_sensor, Units.NONE, DataFormat.FLOAT32, "q0"
            )
        )
        orientation_channels.append(
            self.add_channel_to_db(
                orientation_sensor, Units.NONE, DataFormat.FLOAT32, "q1"
            )
        )
        orientation_channels.append(
            self.add_channel_to_db(
                orientation_sensor, Units.NONE, DataFormat.FLOAT32, "q2"
            )
        )
        orientation_channels.append(
            self.add_channel_to_db(
                orientation_sensor, Units.NONE, DataFormat.FLOAT32, "q3"
            )
        )
        ...
        if version >= 3:  # Also includes magnetometer values
            # Create sensor
            magneto_sensor = self.add_sensor_to_db(
                SensorType.MAGNETOMETER,
                "Magnetometer",
                "AppleWatch",
                "Wrist",
            ...
            mag_channels.append(
                self.add_channel_to_db(
                    magneto_sensor, Units.UTESLA, DataFormat.FLOAT32, "Mag_X"
                )
            )
            mag_channels.append(
                self.add_channel_to_db(
                    magneto_sensor, Units.UTESLA, DataFormat.FLOAT32, "Mag_Y"
                )
            )
            mag_channels.append(
                self.add_channel_to_db(
                    magneto_sensor, Units.UTESLA, DataFormat.FLOAT32, "Mag_Z"
                )
            )
        ...
        for timestamp in motion:
            ...
            count += 1
            self.notify_progress(50 + np.floor(count / len(motion) / 2 * 100))

    def import_battery_to_database(self, settings, sampling_rate, battery: dict):
        # DL Oct. 16 2018, New import to database
        battery_sensor = self.add_sensor_to_db(
            SensorType.BATTERY,
            "Battery",
            "AppleWatch",
            "Wrist",
        ...
        battery_channel = self.add_channel_to_db(
            battery_sensor, Units.VOLTS, DataFormat.UINT8, "Battery Percentage"
        )
        ...
        for timestamp in battery:
            ...
            if len(timesarray) == 0:
                self.notify_error("Aucune donnée temporelle.")
                return
            ...
            count += 1
            self.notify_progress(50 + np.floor(count / len(battery) / 2 * 100))
    ...
    def import_headings_to_database(self, settings, sampling_rate, headings: dict):
        ...
        orientation_channels.append(
            self.add_channel_to_db(
                orientation_sensor, Units.DEGREES, DataFormat.FLOAT32, "True Heading"
            )
        )

        orientation_channels.append(
            self.add_channel_to_db(
                orientation_sensor, Units.DEGREES, DataFormat.FLOAT32, "Accuracy"
            )
        )

        orientation_channels.append(
            self.add_channel_to_db(
                orientation_sensor, Units.DEGREES, DataFormat.FLOAT32, "Mag Heading"
            )
        )
        ...
        for timestamp in headings:
            ...
            count += 1
            self.notify_progress(50 + np.floor(count / len(headings) / 2 * 100))
    ...
    def import_health_to_database(self, settings, health: dict):
        ...
        health_sensor = self.add_sensor_to_db(
        ...
        if "stepCount" in health_types:
            step_sensor = self.add_sensor_to_db(
                SensorType.STEP,
                "Step Count (Health)",
                "AppleWatch",
                "Wrist",
        ...
        for health_type in health_types:
            ...
            if (
                health_type == "activeEnergyBurned"
                or health_type == "basalEnergyBurned"
            ):
                units = Units.KCALORIES
            if (
                health_type == "heartRate"
                or health_type == "respiratoryRate"
                or health_type == "restingHeartRate"
                or health_type == "walkingHeartRateAverage"
            ):
                units = Units.BPM
            if (
                health_type == "oxygenSaturation"
                or health_type == "appleWalkingSteadiness"
            ):
            ...
            if health_type == "bodyTemperature":
                units = Units.DEGREES
            if (
                health_type == "appleExerciseTime"
                or health_type == "appleStandTime"
                or health_type == "appleMoveTime"
            ):
                units = Units.SECONDS
            ...
            # Create channel
            health_channels.append(
                self.add_channel_to_db(
                    base_sensor, units, DataFormat.FLOAT32, health_type
                )
            )
        ...
        for timestamp in health:
            ...
            count += 1
            self.notify_progress(50 + np.floor(count / len(health) / 2 * 100))

    def import_tremor_to_database(self, settings, tremor: dict):
        # Create channels and sensors
        tremor_sensor = self.add_sensor_to_db(
            SensorType.BIOMETRICS,
            "Tremor",
            "AppleWatch",
            "Wrist",
        ...
        tremor_channels.append(
            self.add_channel_to_db(
                tremor_sensor, Units.MILLISECONDS, DataFormat.UINT64, "Start Timestamp"
            )
        )

        tremor_channels.append(
            self.add_channel_to_db(
                tremor_sensor, Units.MILLISECONDS, DataFormat.UINT64, "End Timestamp"
            )
        )

        tremor_channels.append(
            self.add_channel_to_db(
                tremor_sensor, Units.PERCENTAGE, DataFormat.FLOAT32, "None"
            )
        )
        tremor_channels.append(
            self.add_channel_to_db(
                tremor_sensor, Units.PERCENTAGE, DataFormat.FLOAT32, "Slight"
            )
        )
        tremor_channels.append(
            self.add_channel_to_db(
                tremor_sensor, Units.PERCENTAGE, DataFormat.FLOAT32, "Mild"
            )
        )
        tremor_channels.append(
            self.add_channel_to_db(
                tremor_sensor, Units.PERCENTAGE, DataFormat.FLOAT32, "Moderate"
            )
        )
        tremor_channels.append(
            self.add_channel_to_db(
                tremor_sensor, Units.PERCENTAGE, DataFormat.FLOAT32, "Strong"
            )
        )
        ...
        for timestamp in tremor:
            ...
            count += 1
            self.notify_progress(50 + np.floor(count / len(tremor) / 2 * 100))

    def import_dyskinetic_to_database(self, settings, dysk: dict):
        # Create channels and sensors
        dysk_sensor = self.add_sensor_to_db(
            SensorType.BIOMETRICS,
            "Dyskinetic Mvt",
            "AppleWatch",
            "Wrist",
        ...
        dysk_channels = list()
        dysk_channels.append(
            self.add_channel_to_db(
                dysk_sensor, Units.MILLISECONDS, DataFormat.UINT64, "Start Timestamp"
            )
        )

        dysk_channels.append(
            self.add_channel_to_db(
                dysk_sensor, Units.MILLISECONDS, DataFormat.UINT64, "End Timestamp"
            )
        )

        dysk_channels.append(
            self.add_channel_to_db(
                dysk_sensor, Units.PERCENTAGE, DataFormat.FLOAT32, "Likeliness"
            )
        )
        ...
        for timestamp in dysk:
            ...
            count += 1
            self.notify_progress(50 + np.floor(count / len(dysk) / 2 * 100))

    def import_question_to_database(self, settings, question: dict):
        # Create base sensor(s)
        questions_sensor = self.add_sensor_to_db(
            SensorType.QUESTIONS,
            "Prompts",
            "AppleWatch",
            "Wrist",
            0,
        ...
        if "prompts" in settings_json:
            for prompt in settings_json["prompts"]:
                prompt_channels[prompt["id"]] = self.add_channel_to_db(
                    questions_sensor, Units.NONE, DataFormat.JSON, prompt["id"]
                )
        ...
        for timestamp in question:
            ...
            for index in range(timesarray.size):
                ...
                sensor_timestamps = self.create_sensor_timestamps(
                    np.array([question_time, timesarray[index]]), recordset
                )
                ...
                answers_choices = [
                    seta["answer"]["choices"]
                    for seta in settings_json["prompts"]
                    if seta["id"] == answer_id
                ][0]
            ...
            count += 1
            self.notify_progress(50 + np.floor(count / len(question) / 2 * 100))
        ...
        for res in process_list:
            if res.__contains__("motion"):
                ...
                if res["motion"]["timestamps"]:
                    self.import_motion_to_database(
                        res["motion"]["settings"],
                        sampling_rate,
            ...
            if res.__contains__("battery"):
                ...
                if res["battery"]["timestamps"]:
                    self.import_battery_to_database(
                        res["battery"]["settings"],
                        sampling_rate,
            ...
            if res.__contains__("sensoria"):
                ...
                if res["sensoria"]["timestamps"]:
                    self.import_sensoria_to_database(
                        res["sensoria"]["settings"],
                        sampling_rate,
            ...
            if res.__contains__("heartrate"):
                ...
                if res["heartrate"]["timestamps"]:
                    self.import_heartrate_to_database(
                        res["heartrate"]["settings"],
                        sampling_rate,
            ...
            if res.__contains__("beacons"):
                ...
                if res["beacons"]["timestamps"]:
                    self.import_beacons_to_database(
                        res["beacons"]["settings"],
                        sampling_rate,
            ...
            if res.__contains__("coordinates"):
                ...
                if res["coordinates"]["timestamps"]:
                    self.import_coordinates_to_database(
                        res["coordinates"]["settings"],
                        sampling_rate,
            ...
            if res.__contains__("raw_accelero"):
                ...
                if res["raw_accelero"]["timestamps"]:
                    self.import_raw_accelerometer_to_database(
                        res["raw_accelero"]["settings"],
                        sampling_rate,
            ...
            if res.__contains__("raw_gyro"):
                ...
                if res["raw_gyro"]["timestamps"]:
                    self.import_raw_gyro_to_database(
                        res["raw_gyro"]["settings"],
                        sampling_rate,
            ...
            if res.__contains__("pedometer"):
                if res["pedometer"]["timestamps"]:
                    self.import_pedometer_to_database(
                        res["pedometer"]["settings"], res["pedometer"]["timestamps"]
                    )

            if res.__contains__("activity"):
                if res["activity"]["timestamps"]:
                    self.import_activity_to_database(
                        res["activity"]["settings"], res["activity"]["timestamps"]
                    )

            if res.__contains__("headings"):
                sampling_rate = 50  # For now, since no frequency in settings?
                if res["headings"]["timestamps"]:
                    self.import_headings_to_database(
                        res["headings"]["settings"],
                        sampling_rate,
                        res["headings"]["timestamps"],
                    )

            if res.__contains__("health"):
                self.import_health_to_database(
                    res["health"]["settings"], res["health"]["timestamps"]
                )

            if res.__contains__("raw_magneto"):
                sampling_rate = res["raw_magneto"]["sampling_rate"]
                if res["raw_magneto"]["timestamps"]:
                    self.import_raw_magneto_to_database(
                        res["raw_magneto"]["settings"],
                        sampling_rate,
            ...
            if res.__contains__("tremor"):
                if res["tremor"]["timestamps"]:
                    self.import_tremor_to_database(
                        res["tremor"]["settings"], res["tremor"]["timestamps"]
                    )

            if res.__contains__("question"):
                if res["question"]["timestamps"]:
                    self.import_question_to_database(
                        res["question"]["settings"], res["question"]["timestamps"]
                    )

            # Commit DB
            self.db.commit()

    def get_sampling_rate_from_header(
        self, sensor_id, header
    ):  # header = string of json
        sample_rate = 0
        if header != "":
        ...
        if sensor_id in [
            self.RAW_MOTION_ID,
            self.PROCESSED_MOTION_ID,
            self.RAW_ACCELERO_ID,
            self.RAW_GYRO_ID,
        ]:
            sample_rate = 50
    ...
    def read_data_file(self, file, debug=False):
        ...
        if version >= 2:
            ...
            if end_header_id != self.HEADER:
                ...
                self.notify_error(
                    "Format de fichier incorrect - vérifiez qu'il s'agit bien du bon type de fichier."
                )
                return None
        ...
        if sensor_id == self.BATTERY_ID:
            ...
            if os.path.isfile(motion_file_name):
                self.notify_error("Ignoring gyro data - using processed motion instead")
                return None
            ...
            if os.path.isfile(motion_file_name):
                self.notify_error(
                    "Ignoring magneto data - using processed motion instead"
                )
                return None
        ...
        #     dict_name = 'dysk'
            ...
            self.notify_error("Unknown sensor ID:" + str(sensor_id))
            return None
        ...
        results[dict_name]["sampling_rate"] = self.get_sampling_rate_from_header(
            sensor_id, settings_json_str
        )
        ...
        try:
            ...
            while file.readable() and read_data_func is not None:
                ...
                results_ms_data.append(
                    read_data_func(file, debug=debug, version=version)
                )

                if file.seekable():
                    new_progress = np.floor(
                        (file.tell() / self.current_file_size) * 100 / 2
                    )
        ...
        for i, result in enumerate(results_ms_ts):
            ...
            # data
            results[dict_name]["timestamps"][mydate]["values"].append(
                results_ms_data[i]
            )
