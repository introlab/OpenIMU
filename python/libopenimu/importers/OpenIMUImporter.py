...
class OpenIMUImporter(BaseImporter):
    ...
    @timing
    def import_imu_to_database(
        self, timestamp, sample_rate, sensors, channels, recordset, data: dict
    ):
    ...
    @timing
    def import_power_to_database(
        self, timestamp, sensors, channels, recordset, data: dict
    ):
    ...
    def import_gps_to_database(
        ...
        for i, val in enumerate(values):
            ...
            self.add_sensor_data_to_db(
                recordset, sensors["gps"], channels["gps"], sensor_timestamps, geo
            )
    ...
    @timing
    def import_baro_to_database(
        self, timestamp, sensors, channels, recordset, data: list
    ):
        print("import_imu_to_database")
        values = np.array(data["values"], dtype=np.float32)
    ...
    def create_sensor_and_channels(self, sample_rate):
        # Baro
        baro_sensor = self.add_sensor_to_db(
            SensorType.BAROMETER, "Barometer", "OpenIMU-HW", "Unknown", 1, 1
        )

        baro_channel = self.add_channel_to_db(
            baro_sensor, Units.KPA, DataFormat.FLOAT32, "Pressure"
        )

        # Battery
        battery_sensor = self.add_sensor_to_db(
            SensorType.BATTERY, "Battery", "OpenIMU-HW", "Unknown", 1, 1
        )

        battery_channel = self.add_channel_to_db(
            battery_sensor, Units.VOLTS, DataFormat.FLOAT32, "Voltage"
        )

        # Current
        current_sensor = self.add_sensor_to_db(
            SensorType.CURRENT, "Current", "OpenIMU-HW", "Unknown", 1, 1
        )

        current_channel = self.add_channel_to_db(
            current_sensor, Units.AMPERES, DataFormat.FLOAT32, "Current"
        )

        # GPS
        gps_sensor = self.add_sensor_to_db(
            SensorType.GPS, "GPS", "OpenIMU-HW", "Unknown", 1, 1
        )

        gps_channel = self.add_channel_to_db(
            gps_sensor, Units.NONE, DataFormat.UINT8, "GPS_SIRF"
        )
        ...
        gyro_sensor = self.add_sensor_to_db(
            SensorType.GYROMETER, "Gyro", "OpenIMU-HW", "Unknown", sample_rate, 1
        )
        ...
        # Magnetometer
        mag_sensor = self.add_sensor_to_db(
        ...
        mag_channels.append(
            self.add_channel_to_db(
                mag_sensor, Units.UTESLA, DataFormat.FLOAT32, "Mag_X"
            )
        )

        mag_channels.append(
            self.add_channel_to_db(
                mag_sensor, Units.UTESLA, DataFormat.FLOAT32, "Mag_Y"
            )
        )

        mag_channels.append(
            self.add_channel_to_db(
                mag_sensor, Units.UTESLA, DataFormat.FLOAT32, "Mag_Z"
            )
        )
        ...
        for timestamp in results:
            ...
            if results[timestamp].__contains__("imu"):
                ...
                if recordset is not None:
                    if not self.import_imu_to_database(
                        timestamp,
                        sample_rate,
                        sensors,
                        channels,
                        ...
                        self.notify_error("Erreur d'importation données IMU")
                else:
            ...
            if results[timestamp].__contains__("power"):
                ...
                if recordset is not None:
                    if not self.import_power_to_database(
                        timestamp,
                        sensors,
                        channels,
                        recordset,
                        ...
                        self.notify_error("Erreur d'importation données 'Power'")
                else:
            ...
            if results[timestamp].__contains__("gps"):
                ...
                if recordset is not None:
                    if not self.import_gps_to_database(
                        timestamp,
                        sensors,
                        channels,
                        recordset,
                        ...
                        self.notify_error("Erreur d'importation données GPS")
            ...
            if results[timestamp].__contains__("baro"):
                ...
                if recordset is not None:
                    if not self.import_baro_to_database(
                        timestamp,
                        sensors,
                        channels,
                        recordset,
                        ...
                        self.notify_error("Erreur d'importation données barométriques")
                else:
                    print("Barometer - Invalid recordset for timestamp:", timestamp)

            count += 1
            self.notify_progress(50 + np.floor(count / len(results) / 2 * 100))
    ...
    def readDataFile(self, file, debug=False):
            ...
            while file.readable():
                ...
                if headChar[0] == b"h":
                    ...
                    if timestamp_hour is not None:
                        ...
                        results[timestamp_hour]["imu"]["end_time"] = (
                            current_timestamp + 1
                        )
                    ...
                    if timestamp_hour is not None:
                        ...
                        results[timestamp_hour]["gps"]["end_time"] = (
                            current_timestamp + 1
                        )
                    ...
                    if timestamp_hour is not None:
                        ...
                        results[timestamp_hour]["power"]["end_time"] = (
                            current_timestamp + 1
                        )
                    ...
                    if timestamp_hour is not None:
                        ...
                        results[timestamp_hour]["baro"]["end_time"] = (
                            current_timestamp + 1
                        )
        ...
        for timestamp in results:
            for key in results[timestamp]:
                ...
                timevect = np.linspace(
                    results[timestamp][key]["start_time"],
