...
class ExporterTypes:
    ...
    @staticmethod
    def get_icon_for_type(exporter_type_id: int) -> str:
        if exporter_type_id == ExporterTypes.CSV:
            return ":/OpenIMU/icons/file_csv.png"

        if exporter_type_id == ExporterTypes.MATLAB:
            return ":/OpenIMU/icons/file_matlab.png"
...
class DBExporter:
    ...
    def __init__(
        self, db_manager: DBManager, export_path: str, export_format: int, parent=None
    ):
    ...
    def clean_string(to_clean: str) -> str:
        ...
        rval = "".join(
            c
            for c in unicodedata.normalize("NFD", to_clean)
    ...
    def dict_to_csv(filename: str, values: dict):
        with open(filename, "w") as csvfile:
            writer = csv.DictWriter(
                csvfile, fieldnames=values.keys(), delimiter="\t", lineterminator="\n"
            )
            writer.writeheader()
        ...
        for key in values.keys():
            ...
            if isinstance(values[key], dict):
                values[key] = values[key] | {
                    k: 0 for k in values[key].keys() if values[key][k] is None
                }
    ...
    def get_base_path_for_group(self, group_name: str) -> str:
        return (
            ...
            + self.tr("GROUP")
            + "_"
            + DBExporter.clean_string(group_name)
        )

    def get_base_path_for_participant(self, participant: Participant) -> str:
        if participant.group:
            part_dir = (
                self.get_base_path_for_group(participant.group.name)
                + os.sep
                + DBExporter.clean_string(participant.name)
            )
        else:
            part_dir = (
                self.exportPath + os.sep + DBExporter.clean_string(participant.name)
            )
    ...
    def get_base_path_for_recordset(self, recordset: Recordset) -> str:
        return (
            self.get_base_path_for_participant(recordset.participant)
            + os.sep
            + "Recordsets"
            + os.sep
    ...
    def get_base_path_for_processed_data(self, data: ProcessedData) -> str:
        return (
            self.get_base_path_for_participant(
                data.processed_data_ref[0].recordset.participant
            )
            + os.sep
            + "Processed"
            + os.sep
            + DBExporter.clean_string(data.name)
        )
        ...
        if group_name:
            ...
            if group:
                ...
                if self.exportFormat == ExporterTypes.CSV:
                    DBExporter.dict_to_csv(
                        group_dir + os.sep + "infos_Group.csv", group_info
                    )
                if self.exportFormat == ExporterTypes.MATLAB:
                    DBExporter.dict_to_mat(
                        group_dir + os.sep + "infos_Group.mat", {"group": group_info}
                    )
    ...
    def export_participant(self, id_participant: int):
        ...
        if participant:
            ...
            if self.exportFormat == ExporterTypes.CSV:
                DBExporter.dict_to_csv(
                    part_dir + os.sep + "infos_Participant.csv", part_info
                )
            if self.exportFormat == ExporterTypes.MATLAB:
                DBExporter.dict_to_mat(
                    part_dir + os.sep + "infos_Participant.mat",
    ...
    def export_recordset(self, id_recordset: int):
        ...
        if recordset:
            ...
            infos = {
                ...
                "end_time": str(recordset.end_timestamp),
                "end_timestamp": recordset.end_timestamp.timestamp(),
            }
            if self.exportFormat == ExporterTypes.CSV:
                DBExporter.dict_to_csv(rec_dir + os.sep + "infos_Recordset.csv", infos)
            if self.exportFormat == ExporterTypes.MATLAB:
                DBExporter.dict_to_mat(
                    rec_dir + os.sep + "infos_Recordset.mat", {"recordset": infos}
                )
            ...
            for sensor in sensors:
                # Do something
                all_data = self.dbMan.get_all_sensor_data(
                    recordset=recordset, sensor=sensor
                )
    ...
    def export_processed_data(self, id_processed_data: int):
        ...
        if data:
            ...
            # Export infos in file
            infos = {
                "id_processed_data": data.id_processed_data,
                "name": data.name,
                "id_data_processor": data.id_data_processor,
                "params": data.params,
                "recordsets": [
                    {"id_recordset": r.recordset.id_recordset, "name": r.recordset.name}
            ...
            if self.exportFormat == ExporterTypes.CSV:
                DBExporter.dict_to_csv(
                    results_dir + os.sep + "infos_" + clean_data_name + ".csv", infos
                )
            if self.exportFormat == ExporterTypes.MATLAB:
                DBExporter.dict_to_mat(
                    results_dir + os.sep + "infos_" + clean_data_name + ".mat",
            ...
            if results:
                ...
                if "result" in results[0].keys():
                    ...
                    result_variables = [
                        var
                        for var in results[0]["result"].keys()
                ...
                for idx, result in enumerate(results):
                    array = [
                        result[key] for key in header if not key.startswith("result_")
                    ]
                    if result_variables:
                        array.extend(
                            [result["result"][var] for var in result_variables]
                        )
    ...
    def export_sensor_data(
        self, sensor: Sensor, sensors_data: list[SensorData], base_dir: str
    ):
        result = {}
    ...
    def export_sensor_data_questions(
        self, sensor: Sensor, sensors_data: list[SensorData], base_dir: str
    ):
        ...
        for sensor_data in sensors_data:
            ...
            answers_obj[index][3] = ", ".join(
                [str(item) for item in data["answer_index"]]
            )
    ...
    def export_sensor_data_health(
        ...
        for id_channel in result.keys():
            ...
            if channel:
                label = (
                    channel.label.replace(" ", "_").replace("(", "").replace(")", "")
                )
                time = []
                values = []
            ...
            if self.exportFormat == ExporterTypes.CSV:
                ...
                sio.savemat(
                    filename + ".mat",
                    {
                        sensor.name.replace(" ", "")
                        + "_"
                        + label: {"values": my_array.transpose(), "labels": header}
                    },
                    do_compression=True,
                    long_field_names=True,
                )

    def export_sensor_data_beacons(
        self, sensor: Sensor, sensors_data: list[SensorData], base_dir: str
    ):
        result = {}
        for sensor_data in sensors_data:
            ...
            if channel:
                ...
                for data in result[id_channel]:
                    if beacons[beacon_id] is None:
                        rows = len(data["time"])
                        beacons[beacon_id] = np.array(
                            [data["time"], [None] * rows, [None] * rows]
                        ).transpose()
                    ...
                    if channel.label.endswith("RSSI"):
                        for index, t in enumerate(data["time"]):
                            ...
                            if t_index:  # Time already there?
                                ...
                                beacons[beacon_id] = beacons[beacon_id] = np.vstack(
                                    (
                                        beacons[beacon_id],
                    ...
                    if channel.label.endswith("Power"):
                        for index, t in enumerate(data["time"]):
                            ...
                            if t_index:  # Time already there?
                                ...
                                beacons[beacon_id] = np.vstack(
                                    (
                                        beacons[beacon_id],
                                        [t, None, data["values"][index]],
                                    )
                                )
        del result
    ...
    def export_sensor_infos(self, sensor: Sensor, base_dir: str, base_filename: str):
        infos = {
            ...
            "channels": len(sensor.channels),
            "hardware_id": sensor.hw_id,
            "hardware_name": sensor.hw_name,
            "settings": sensor.settings,
        }
        if self.exportFormat == ExporterTypes.CSV:
            DBExporter.dict_to_csv(
                base_dir + os.sep + "infos_" + base_filename + ".csv", infos
            )
        if self.exportFormat == ExporterTypes.MATLAB:
            DBExporter.dict_to_mat(
                base_dir + os.sep + "infos_" + base_filename + ".mat", {"sensor": infos}
            )
