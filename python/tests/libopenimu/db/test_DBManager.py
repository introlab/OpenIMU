...
class DBManagerTest(unittest.TestCase):
    ...
    def test_add_sensor(self):
        ...
        sensor = manager.add_sensor(
            id_sensor_type, name, hw_name, location, sampling_rate, data_rate
        )
    ...
    def test_add_participant(self):
        ...
        participant = Participant(
            name=participant_name, description=participant_description, group=group
        )
    ...
    def test_get_all_recordsets(self):
        ...
        for i in range(0, count):
            ...
            recordsets1.append(
                manager.add_recordset(
                    participant1, "Record Name" + str(i), time1, time2
                )
            )
            recordsets2.append(
                manager.add_recordset(
                    participant2, "Record Name" + str(i), time1, time2
                )
            )
        ...
        self.assertEqual(
            len(manager.get_all_recordsets()), len(recordsets1) + len(recordsets2)
        )
    ...
    def test_add_sensor_data(self):
        ...
        recordset = manager.add_recordset(
            participant,
            "My Record",
    ...
    def test_get_all_sensor_data_with_args(self):
        ...
        sensor2 = manager.add_sensor(
            SensorType.GYROMETER, "Sensor Name", "Hardware Name", "Wrist", 30.0, 1
        )
        ...
        sensordata_res = manager.get_all_sensor_data(
            recordset=recordset, convert=True, sensor=sensor
        )
        self.assertEqual(len(sensordata_res), 2)
        for sensor_data in sensordata_res:
            self.assertEqual(len(sensor_data.data), len(data))
        ...
        sensordata_res = manager.get_all_sensor_data(
            recordset=recordset, convert=True, sensor=sensor2
        )
        ...
        sensordata_res = manager.get_all_sensor_data(
            recordset=recordset, convert=True, channel=channel1
        )
        ...
        sensordata_res = manager.get_all_sensor_data(
            recordset=recordset, convert=True, channel=channel2
        )
        self.assertEqual(len(sensordata_res), 1)
        for sensor_data in sensordata_res:
            self.assertEqual(len(sensor_data.data), len(data))
        ...
        sensordata_res = manager.get_all_sensor_data(
            recordset=recordset, convert=True, channel=Channel()
        )
        self.assertEqual(len(sensordata_res), 0)
